...
getNnTmpTable <- function(xyzTest, xyzTrain, classTest, classTrain) {
  nnRes <- neuralnet(outcome ~ ., cbind(xyzTrain, outcome = classTrain), hidden = 5, rep = 1, stepmax=1e+06, threshold = 0.1, linear.output = FALSE, err.fct = 'ce')
  nnTmpTbl <- table(classTest, round(predict(nnRes, newdata = xyzTest)))
  return (nnTmpTbl)
}

nnTable <- getNnTmpTable(xyzTest, xyzTrain, classTest, classTrain)
