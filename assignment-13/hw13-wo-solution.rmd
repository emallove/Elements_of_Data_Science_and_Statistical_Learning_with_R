...
```{r neuralNet_1HiddenLayerOfSixNodes}

computeErrorRate <- function (myTable) {
  return (1 - sum(diag(myTable)) / sum(myTable))
}

getNnTmpTable <- function(xyzTest, xyzTrain, classTest, classTrain, hiddenLayerNodesArg) {
  nnRes <- neuralnet(outcome ~ ., cbind(xyzTrain, outcome = classTrain), hidden = c(hiddenLayerNodesArg), linear.output = FALSE)
  #print(nnRes)
  #plot(nnRes)
...
for (hiddenLayerNode in 1:6) {
  nnTable <- getNnTmpTable(xyzTest, xyzTrain, classTest, classTrain, hiddenLayerNode)
  print(sprintf("Number of nodes in hidden layer: %d", hiddenLayerNode))
  computeErrorRate(nnTable)
}
