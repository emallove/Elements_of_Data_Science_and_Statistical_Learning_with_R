...
simuLinQuadDat <- function(inpNobs=100, inpNlinVars=5, inpYidx=1:2, inpSDerr=0.5) {
  ...
  for ( iTmp in 1:ncol(xTmp) ) {
    ...
    for ( jTmp in iTmp:ncol(xTmp) ) {
      x2SimuLinTmp <- cbind(x2SimuLinTmp,xTmp[,iTmp]*xTmp[,jTmp])
...
```{r p1,fig.width=12,fig.height=4}
t <- read.table("fund-raising-with-predictions.csv", sep=",", header=TRUE)
t2 <- log(t[1:12] + 1)
# library(corrgram)
myNames <- names(sort(abs(cor(t2)[1,]), decreasing = TRUE))
t3 <- t2[myNames]
```
...
```{r, sp2}
  ...
  for ( iTmp in 1:ncol(xTmp) ) {
    ...
    # excluding already generated pairwise combinations: 
    for ( jTmp in iTmp:ncol( xTmp) ) {
      x2Tmp <- cbind( x2Tmp, log(xTmp[, iTmp] * xTmp[, jTmp] + 1))
      # maintain vector of column names for quadratic
      # terms along the way:
      tmpCnms <- c( tmpCnms, paste0( xTmpNames[iTmp], "_x_", xTmpNames[jTmp]))
    }
  }
  # name attributes in the matrix of quadratic terms:
  colnames(x2Tmp) <- c(xTmpNames, tmpCnms)
```
...
```{r, sp3}
df2plot <- NULL
for ( iTmp in 2:ncol( x2Tmp) ) {
  lmTmp <- lm( contrib~., as.data.frame(x2Tmp)[ , 1:iTmp])
  errTmp <- sqrt( mean(( x2Tmp[ , "contrib"] - predict( lmTmp))^2))
  df2plot <- rbind( df2plot, data.frame( nvars = iTmp - 1, err = errTmp))
}
plot( df2plot, xlab = "Number of variables", ylab = "Regression error", main = paste( nrow( x2Tmp), "observations"), ylim = c( min( df2plot[, "err"]), sqrt( mean(( x2Tmp[, "contrib"] - mean( x2Tmp[, "contrib"]))^2))))
abline( h = sqrt( mean(( x2Tmp[, "contrib"] - mean( x2Tmp[, "contrib"]))^2)), lty = 2)
```
...
```{r bootTrainTestErrOneAllVars2}
bootTrainTestErrOneAllVars2 <- function(inpDat,nBoot=100,colOne) {
  # matrices and vector to store bootstrap training
  # and test errors as well as training error for model
  # fit on all observations -- for one through all
  # variables in the dataset:
  ...
  allTrainErr <- numeric()
  # first predictor is the second column in
  # the input data - first is the outcome colOne:
  for ( iTmp in 2:ncol(inpDat) ) {
    # fit model and calculate error on all observations:
...
```{r bootErr200Two,fig.width=6,fig.height=6}
# wrapper for plotting:
plotBootRegrErrRes <- function(inpRes,inpPchClr = c(1,2,4),...) {
  matplot(1:length(inpRes$allTrain),cbind(inpRes$allTrain,colMeans(inpRes$bootTrain),colMeans(inpRes$bootTest)),pch = inpPchClr,col = inpPchClr,lty = 1,type = "b",xlab = "Number of predictors",ylab = "Regression error",...)
  legend("topright",c("train all","train boot","test boot"),col = inpPchClr,text.col = inpPchClr,pch = inpPchClr,lty = 1)
}
bootErrRes <- bootTrainTestErrOneAllVars2(x2Tmp, 30, "contrib")
plotBootRegrErrRes(bootErrRes, main = "200 observations", ylim = c(min(colMeans(bootErrRes$bootTrain)), sqrt(mean((mean(x2Tmp[, "contrib"]) - x2Tmp[, "contrib"])^2))))
abline(h = sqrt(mean((mean(x2Tmp[, "contrib"]) - x2Tmp[, "contrib"])^2)), lty = 2)
```
...
```{r simulThreeSz2, fig.width=12, fig.height=4}
old.par <- par(mfrow = c(1, 3))
for ( tmpNobs in c(200, 500, 1000) ) {
  bootErrRes <- bootTrainTestErrOneAllVars2(x2Tmp, 30, "contrib")
  plotBootRegrErrRes(bootErrRes, main = paste(tmpNobs, "observations"), ylim = c(min(colMeans(bootErrRes$bootTrain)), sqrt(mean((mean(x2Tmp[, "contrib"]) - x2Tmp[, "contrib"])^2))))
  abline(h = sqrt(mean((mean(x2Tmp[, "contrib"]) - x2Tmp[, "contrib"])^2)), lty = 2)
}
par(old.par)
```
...
for ( tmpNobs in c(200, 500, 1000) ) {
  x3Tmp <- x2Tmp[tmpNobs, ]
  bootErrRes <- bootTrainTestErrOneAllVars2(x3Tmp, 30, "contrib")
  plotBootRegrErrRes(bootErrRes, main = paste(tmpNobs, "observations"), ylim = c(min(colMeans(bootErrRes$bootTrain)), sqrt(mean((mean(x3Tmp[, "contrib"]) - x3Tmp[, "contrib"])^2))))
  abline(h = sqrt(mean((mean(x3Tmp[, "contrib"]) - x3Tmp[, "contrib"])^2)), lty = 2)
}
par(old.par)
