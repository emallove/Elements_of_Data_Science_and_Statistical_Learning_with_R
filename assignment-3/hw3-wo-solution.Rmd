...
> 
> 2. Fit linear model for target campaign contribution as the outcome and the last contribution by this donor (`lastcontr` in `fund-raising.csv`) the predictor, using R function `lm`; inspect the fitted model using `summary` function, ... 

```{r fitLinearModel}
old.par <- par(mfrow=c(2,2))
plot(lm(lastcontr~contrib,frcDat))
```
...
Residual standard error: 7.22 on 3468 degrees of freedom
Multiple R-squared:  0.5572,	Adjusted R-squared:  0.5571    

> Remember, you can find them in the `summary` output or use `sigma` and `r.sq` slots in the result returned by `summary` instead (the `summary()` command does return a *list*; if instead of just printing the result into the console you save it into a variable, as in `model.summary <- summary(...)`, you can verify that the content of that variable *is* a list, you can see with `names(model.summary)` which elements this list contains, and you can extract, examine, and use them at will if you ever need to)
   
```{r fitLinearModel-summary}
old.par <- par(mfrow=c(2,2))
summary(lm(lastcontr~contrib,frcDat))
```
   
> + What are the model coefficients and what would be their interpretation? What is the meaning of the intercept of the model, for example?  
   
The intercept is the Y value if X is equal to 0.  In the case where X is never 0, the intercept is meaningless.
   
   Coefficients:
            Estimate Std. Error t value Pr(>|t|)    
(Intercept)  4.08639    0.20332   20.10   <2e-16 ***
contrib      0.70067    0.01061   66.06   <2e-16 ***
   
> What about the slope - how would you interpret its value?
...
```{r targetCampaignContributionAndLastContributionPlot}
plot(frcDat[,c("lastcontr", "contrib")])
abline(lm(lastcontr~contrib,frcDat),col=2,lwd=2)
abline(h=0,lty=2)
abline(v=0,lty=2)
```
...
```{r DiagPlots,fig.width=4,fig.height=4}
old.par <- par(mfrow=c(2,2))
plot(lm(lastcontr~contrib,frcDat))
par(old.par)
```
...
```{r nAveContrIntls2}
confint(lm(log10(avecontr)~log10(ncontrib),frcDat))
10^predict(lm(lastcontr~contrib,frcDat), newdata=data.frame(contrib=c(9,10,11)), interval='confidence')
10^predict(lm(lastcontr~contrib,frcDat), newdata=data.frame(contrib=c(9,10,11)), interval='prediction')
```
...
A prediction interval is a likelihood window containing response values a new experimental observation for given predictor settings. The confidence interval of the prediction is a likelihood window containing the mean response.

>     + comment on whether confidence or prediction intervals (on predictions) are wider and why

The confidence interval tells you the location of the population parameter. Prediction intervals explain both the uncertainty in knowing the value of the mean, plus random scatter. So a prediction interval is always wider than a confidence interval.

    
```{r predictLastContrContrib}
10^predict(lm(lastcontr~contrib,frcDat), newdata=data.frame(contrib=c(10,20,40)), interval='confidence')
10^predict(lm(lastcontr~contrib,frcDat), newdata=data.frame(contrib=c(10,20,40)), interval='prediction')
```
...
```{r logTransformedForLastContrEq0}
summary(lm(log10(subset(frcDat$lastcontr, frcDat$lastcontr != 0)) ~ log10(subset(frcDat$contrib, frcDat$lastcontr != 0)), frcDat))
```
...
```{r nAvePlot2}
plot(log10(frcDat[,c("lastcontr","contrib")]))
abline(lm(log10(subset(frcDat$lastcontr, frcDat$lastcontr != 0)) ~ log10(subset(frcDat$contrib, frcDat$lastcontr != 0)), frcDat), col=2, lwd=2)
abline(h=0,lty=2)
abline(v=0,lty=2)
```
...
```{r diagnosticPlotsForLogTransformedOutcome,fig.width=8,fig.height=8}
old.par <- par(mfrow=c(2,2))
plot(lm(log10(subset(frcDat$lastcontr, frcDat$lastcontr != 0)) ~ log10(subset(frcDat$contrib, frcDat$lastcontr != 0)), frcDat))
par(old.par)
```
...
```{r logTransformedForLastContrEq02}
summary(lm(log10(subset(frcDat$lastcontr, frcDat$lastcontr != 0)) ~ (log10(subset(frcDat$contrib, frcDat$lastcontr != 0)) + log10(subset(frcDat$avecontr, frcDat$lastcontr != 0)))))
```

```{r diagnosticPlotsForLogTransformedOutcome2,fig.width=8,fig.height=8}
old.par <- par(mfrow=c(2,2))
plot(lm(log10(subset(frcDat$lastcontr, frcDat$lastcontr != 0)) ~ (log10(subset(frcDat$contrib, frcDat$lastcontr != 0)) + log10(subset(frcDat$avecontr, frcDat$lastcontr != 0))), frcDat))
par(old.par)
```