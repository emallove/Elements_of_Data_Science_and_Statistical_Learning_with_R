...
plot(pcaResTmp$x[,1:2],type="p",main="Original",col=cids)
...
```{r prcompScaled}
pcaResTmpScaled <- prcomp(scale(USArrests))
cidsScaled <- kmeans(scale(USArrests),centers=4,nstart=10)$cluster
plot(pcaResTmpScaled$x[,1:2],type="p",main="Scaled",col=cidsScaled)
plot(pcaResTmpScaled$x[,1:2],type="n",main="Scaled",col=cidsScaled)
text(pcaResTmpScaled$x[,1:2],rownames(pcaResTmpScaled$x),col=cidsScaled,pch=15+cidsScaled)
# par(old.par)
```
...
ins_random_fun <- function(x) {
    runif(length(x), min(x), max(x));
}
# debug(ins_random_fun)

for ( i in 1:10 ) {
  wrnd = numeric()
  for ( k in 1:20 ) {
    krnd = kmeans(apply(USArrests, 2, ins_random_fun), k)
