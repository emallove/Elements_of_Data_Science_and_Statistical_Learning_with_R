...
_There's significant overlap of signal strengths across the four rooms, though the plots are distinct.  One would need many () readings (from apparently different parts of the room) to reduce the error rate._
...
_For a stricter scenario, we could use triangulation of signal strengths from three routers to give a precise position (a la GPS systems)._

_For a more lax scenario would simply allow for a prediction of a room pair (instead of a single room).  Or allow for a series of signal strength readings, instead of just one._
...
```{r fund-raising}
d <- matrix(
      scan("fund-raising.csv",
           skip=1,
           sep=",",
           what=list('numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','character')
      ),
      ncol=13,
      byrow=TRUE
)

paste("Dimensions ", dim(d))
paste("Rows: ",      nrow(d))
paste("Columns: ",   ncol(d))
...
```{r fund-raising-pair-plots1}
pairs(c(d[,1:3]))
```

```{r fund-raising-pair-plots2}
pairs(c(d[,3:6]))
```

```{r fund-raising-pair-plots3}
pairs(c(d[,6:8]))
```
